...
while True:
	...
	#taking size of drive from user 
	data1=s.recvfrom(100)
	d_size=data1[0]

	c_addr=data1[1][0]
	
	#creating VG pool/thin pool
	#os.system('lvcreate --size 3000M --thin '+xyz/pool1)

	#creating LVM by the name of client drive
	os.system('lvcreate -V'+d_size+' --name '+d_name+' --thin  xyz/pool1')
